...
interface RestoreAttachmentBody {
  attachmentId?: string
}
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<RestoreAttachmentBody>(event)
  ...
  await logAuditAction(callerId, 'attachment_restored', eventId, { attachmentId: attachment.id })
